...
\begin{frame}
  ...
  \begin{tikzpicture}
    ...
    \foreach \y in {0,...,\Ny} {
      \pgfmathtruncatemacro{\yy}{1+\Ny+\Ny-\y}
      \pgfmathtruncatemacro{\ilim}{2*(\Nx+1)-1}
      \foreach \i in {0,...,\ilim} {
        \node [draw,fill=blue!20,minimum height=1cm,minimum
          width=1cm] (rama\y\i) at
          (-2.2-1*\i-1*\Ny+1*\y,1.2*\y) {\tiny A[\yy][\i]};
      }
    ...
    \foreach \x in {0,...,\Nx} {
      ...
      \foreach \i in {0,...,\ilim} {
        \node [draw,fill=green!20,minimum height=1cm,minimum
          width=1cm] (ramb\x\i) at
          (1.2*\x,1.2*\Ny+2+1*\i+1*\x) {\tiny B[\i][\x]};
      }
